...
class Sequence_model(db.Model):
    ...
    conversions = db.Column(db.String, nullable=False)
    translation_table = db.Column(db.String, nullable=False)
    creationDate = db.Column(
        db.DateTime, nullable=False, default=datetime.datetime.utcnow)

    def __repr__(self):
        return f"sequencia {self.seq_id} ; {self.seq_type} ; {self.sequence} ; {self.conversions} ; {self.translation_table} ; {self.creationDate}"
